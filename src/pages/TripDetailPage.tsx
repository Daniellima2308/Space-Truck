...
function computeSmartBanner(
  ...
  if (activeFreight && hasEstimatedKm) {
    return {
      ...
      message:
        "O Space Truck está usando a rota cadastrada até entrar mais KM real. Isso ajuda a acompanhar sem travar a operação.",
    };
  }
            ...
            <MetricCard
              ...
              onClick={() =>
                setActiveDetail({
                  ...
                  description:
                    (isOpen ? kmBasisToDate.km : kmBasisTotal.km) > 0
                      ? `${hasEstimatedKm ? "Esse valor ainda pode mudar durante a viagem. " : ""}${isOpen && kmBasisToDate.source === "estimated" ? "A base de KM considera só trechos operacionais, sem incluir fretes planejados. " : ""}Como o Space Truck chegou nesse valor.`
                      : "Aguardando KM da viagem.",
                  lines:
                    (isOpen ? kmBasisToDate.km : kmBasisTotal.km) > 0
                      ? [
