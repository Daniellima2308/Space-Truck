...
| --- | --- | --- |
| `README.md` | Ainda parece baseado no modelo Lovable e contém instruções genéricas de edição/deploy. | Reescrever em PR futura com foco no Space Truck, setup local, variáveis de ambiente e fluxo de contribuição. |
| `src/pages/Index.tsx` | Parece fallback antigo com texto de app em branco. | Validar rotas e referências antes de remover. Se não for usado, remover em PR própria. |
...
| `src/lib/storage.ts` | Parece storage legado em `localStorage`, incluindo chave antiga `estrada-real-data`. | Confirmar se ainda há importações ou dependência de migração local antes de remover. |
...
| Storybook | A estrutura está bem configurada e já possui separação entre componentes base e patterns. | Evoluir depois da limpeza da base, priorizando padrões reais do app e evitando telas fake. |
